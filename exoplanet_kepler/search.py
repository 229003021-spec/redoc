...
def coarse_fine_bls_search(t: np.ndarray, f: np.ndarray, grid_type: str = "uniform_freq", verbose: bool = False) -> dict:
    ...
    pmax = min(PERIOD_MAX, baseline / 2.0)  # Relaxed period cut allowing long-period candidates up to baseline / 2
    ...
    if grid_type == "uniform_freq":
        # Dense frequency grid: dfreq = 1 / (8 * baseline) => ultra-fine resolution for P > 80-100 days
        f_min = 1.0 / pmax
        f_max = 1.0 / PERIOD_MIN
        dfreq = 1.0 / (8.0 * baseline)
        n_freqs = min(N_COARSE, max(4000, int((f_max - f_min) / dfreq)))
